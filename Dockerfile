...
RUN dotnet publish -c Release -r ${TARGETOS}-${TARGETARCH}
...
COPY --from=backend --chown=lmmentor:lmmentor /src/LMMentor.Backend/bin/Release/net10.0/publish/${TARGETOS}-${TARGETARCH}/app /app
USER lmmentor
WORKDIR /data
EXPOSE 8080
VOLUME ["/data"]
